...
Beyond membership inference, a deployed authentication system faces a second threat: an adversary who wishes to be accepted as a legitimate user. Projected gradient descent (PGD) attacks can perturb an impostor's gait signal to minimise the system's output distance to a target member, potentially crossing the decision boundary with a perturbation small enough to remain within the natural variation of human gait. Whether such perturbations are practically achievable, and whether they leave detectable spectral artefacts, is an open question for gait-based systems that has received less attention than the equivalent question in image-domain authentication.
...
\begin{itemize}
  ...
  A PGD impersonation attack is conducted on all four dataset configurations. The minimum perturbation budget required for a successful attack is compared against the median intra-subject gait variation, providing a naturalness baseline. Spectral analysis of the successful perturbations is included for the whuGAIT configuration.
...
Chapter~\ref{ch:mia} presents the membership inference experiments: the simple-delta baseline, the LiRA shadow model attack in its three threat model variants, the delta formulation comparison, and a discussion of the whuGAIT pair-correlation confound.

Chapter~\ref{ch:signal} presents the signal disentanglement experiment: the three-group design, the authentication model performance per group, and the per-group membership signal comparison across attack variants.
